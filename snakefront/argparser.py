...
from snakefront import settings
...
templates = os.path.join(here, "..", "main", "templates", "forms")
...
class SnakefaceParser:
    ...
    snakefiles = None
    def set_snakefiles(self, _snakefiles):
        print('SnakefaceParser', _snakefiles)
        self.snakefiles = _snakefiles
    ...
    def __init__(self, _snakefiles=None):
        """load the parser, optionally specifying a profile"""
        if _snakefiles == None:
            self.snakefiles = get_snakefile_choices()
        else:
            self.snakefiles = _snakefiles
        self.parser = get_argument_parser()
        self._groups = {}
    ...
    def groups(self):
        ...
        for action in self.parser._actions:
            ...
            if action.dest == "snakefile":
                print('groups','snakefile',self.snakefiles)
        ...
        # print('SnakeFaceParser', 'groups', self._groups['EXECUTION'].keys())
        return self._groups
    ...
    def include_argument(self, name, group):
        ...
        if cfg.EXECUTOR_CLUSTER and group == "CLUSTER":
            return False
        if cfg.EXECUTOR_GOOGLE_LIFE_SCIENCES and group == "GOOGLE_LIFE_SCIENCE":
            return False
        if cfg.EXECUTOR_KUBERNETES and group == "KUBERNETES":
            return False
        if cfg.EXECUTOR_TIBANNA and group == "TIBANNA":
            return False
        if cfg.EXECUTOR_TIBANNA and group == "TIBANNA":
            return False
        if cfg.EXECUTOR_GA4GH_TES and group == "TES":
            return False
